...
graph LR
    ...
    I -->|execution block| J[Errand created]
    J -->|grouped under| MA[Matter]
    J --> K[Task Runner]
    K -->|sessions_spawn| L[Execution]
    L --> M[Ledger entry]
    L -->|consequentials| J
```

A single input can be routed by Judgment, processed by the Curator, triaged for human review, trigger an errand via an instinct's execution block, and spawn follow-up errands — all without you doing anything beyond sharing the content.
