...
|---|---|---|
| Stream events (Composio, webhooks, scheduled pulls) | Calendar events, emails, GitHub PRs, Slack messages, Notion pages, payments, Telegram messages, voice-call transcripts | Zero per event; one per ~200 events per hour |
...
Once an event lands in the JSONL, `EventProcessorWorkflow` picks it up every two minutes (`packages/learn/src/workflows/event_processor.py`) and dispatches based on `stream_type`:
...
graph LR
    ...
    B -->|everything else| TPL[Python template → vault event/ record]
...
**Stream events** (calendar, email, GitHub, Slack, Notion, payment, SMS, voice-call) take the zero-LLM path. `stream_vault.py` dispatches the event to a per-source Python template:
...
Each template returns a `(name, body, tags)` tuple. The result is written as a vault `event/` record with `enrichment_status: pending` in its frontmatter. Total cost: zero LLM calls, a few milliseconds per event.
...
<Tip>
The two-minute event-processor tick and the one-hour enrichment tick are deliberate. New events show up in the vault and the dashboard within a couple of minutes, so the agent can answer "what just happened?". Enrichment fills in the entities and tags within the hour, so the agent can answer "who's involved?" by the next time Sir asks.
</Tip>
...
- **Anyone else** is dropped into the same stream pipeline as everything else: a vault `event/` record via the email/sms/voice-call template, picked up by hourly enrichment, no reply.
