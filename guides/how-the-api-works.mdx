...
description: "Understand the secure proxy architecture that connects your API calls to your Alfred."
icon: "route"
---
...
sequenceDiagram
    ...
    participant SaaS as SaaS Proxy<br/>(alfred.black)
    participant TS as Tailscale Network
    participant Tenant as Your Alfred<br/>(alfred-ctrl)
    participant Docker as Services

    Script->>SaaS: POST /api/v1/vault/inbox<br/>Authorization: Bearer alf_xxx
    SaaS->>SaaS: Hash key (SHA-256)<br/>Look up ApiKey row<br/>Find your Alfred
    SaaS->>SaaS: Decrypt internal key<br/>(AES-256-GCM)
    SaaS->>TS: Forward request via Tailscale<br/>https://{hostname}:3100/api/v1/vault/inbox<br/>Authorization: Bearer {internal_key}
    TS->>Tenant: Deliver to your Alfred
    Tenant->>Tenant: Authenticate with internal key
...
You create API keys in your [dashboard](https://alfred.black) under **Settings > API Keys**. Each key starts with `alf_` followed by 32 hex characters:
...
|-------|----------------|
| Alfred exists | `404` — No Alfred found |
| Tailscale hostname and internal key are set | `503` — Not ready yet |
| Status is `running` | `503` — Not running |

### 4. The internal key is decrypted

Your Alfred's internal API key (used for communication between the SaaS and your Alfred) is stored encrypted using AES-256-GCM. The proxy decrypts it at request time using a server-side encryption key.
...
| No Alfred provisioned | `404` | No instance found |
| Not ready or not running | `503` | Instance is not ready / not running |
| Request timed out (15s) | `504` | Request timed out |
| Unreachable | `502` | Failed to reach instance |
