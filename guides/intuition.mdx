...
The Task Runner picks up the task, assembles context, and spawns an **ephemeral agent** on the workers runtime. This agent has access to exactly the tools it needs — including `composio_execute` for actions on connected apps. When the task completes, the ephemeral agent is destroyed and a ledger entry is written.

**Example**: you tell Alfred *"from now on, when a payment failure email arrives, reply acknowledging receipt."* Alfred captures this as an observation. Reflection creates an instinct with an execution block specifying `tools_required: ["GMAIL_REPLY_TO_EMAIL"]`. When a matching email arrives, Judgment fires, the Task Runner spawns an ephemeral agent, and that agent calls `composio_execute` to send the reply via your connected Gmail — without your intervention.
...
This closes the loop from observation to action: you teach Alfred by routing, Alfred learns through reflection, and eventually Alfred both routes *and* acts on connected apps — with appropriate caution.
...
|---------|----------|-------------|
| **Event Processor** | Every 15 minutes | Reads incoming stream events, creates vault records via zero-LLM templates |
| **Hourly Enrichment** | Every 1 hour | Batched LLM call to add entities, tags, and priorities to recent event records |
...
| **Task Runner** | Every 15 minutes | Picks up queued errands, spawns ephemeral agents for execution on connected apps |
