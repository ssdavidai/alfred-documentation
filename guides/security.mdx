...
Set correct file permissions before connecting — `chmod 600` on macOS/Linux, or `icacls` on Windows (see the [Dashboard guide](/guides/your-dashboard#ssh-access) for full instructions).

### Key security

| Property | Detail |
|----------|--------|
| **Generation** | Keys are generated per-tenant during instance provisioning |
| **Storage** | Private keys are stored encrypted on the platform and delivered over HTTPS |
| **Isolation** | Keys are never shared between tenants — each instance has its own key pair |
| **Access level** | SSH provides full access to the tenant VPS as the `deploy` user |
