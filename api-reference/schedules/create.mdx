...
Creates a new workflow schedule via `temporal schedule create`. Returns 201 with a confirmation message.
...
<Note>
  The response may include a `raw` field containing the Temporal CLI's stdout if there was any output from the create command.
</Note>
