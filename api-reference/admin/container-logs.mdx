---
title: "Get Container Logs"
description: "Retrieve logs from a specific container (non-streaming)"
api: "GET /api/v1/admin/containers/{service}/logs"
---

Retrieve historical logs from a specific container. This endpoint returns a snapshot of recent logs and does not stream updates.

## Path Parameters

<ParamField path="service" type="string" required>
  The service name to get logs from (e.g., "worker", "openclaw", "temporal"). Must contain only alphanumeric characters, hyphens, or underscores.
</ParamField>

## Query Parameters

<ParamField query="tail" type="number" optional default="100">
  Number of log lines to retrieve from the end of the log file.
</ParamField>

<RequestExample>

```bash curl
# Get last 100 lines from worker logs (default)
curl -H "X-API-Key: YOUR_API_KEY" \
  https://alfred-acme.tailnet.ts.net:3100/api/v1/admin/containers/worker/logs

# Get last 500 lines from OpenClaw logs
curl -H "X-API-Key: YOUR_API_KEY" \
  "https://alfred-acme.tailnet.ts.net:3100/api/v1/admin/containers/openclaw/logs?tail=500"
```

</RequestExample>

<ResponseExample>

```json
{
  "logs": "2026-02-26T10:15:20Z INFO - Alfred orchestrator starting\n2026-02-26T10:15:21Z INFO - Starting curator daemon\n2026-02-26T10:15:22Z INFO - Starting janitor daemon\n2026-02-26T10:15:23Z INFO - Starting distiller daemon\n2026-02-26T10:15:24Z INFO - All daemons running\n"
}
```

</ResponseExample>

<Note>
  For real-time log streaming, use the `/api/v1/logs` SSE endpoint instead. This endpoint is designed for quick log inspection and debugging.
</Note>
