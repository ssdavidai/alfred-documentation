...
```mermaid
graph TB
    L6["<b>Interface</b><br/>Dashboard · Channels · Talk · Ambient"]
    L5["<b>Kinetic</b><br/>Temporal · Tasks · Chores · Rules"]
    L4["<b>Semantic</b><br/>Vault · 22 Record Types · Intuition"]
    L3["<b>Data</b><br/>Streams · Events · Session Logs"]
...
OpenClaw hosts four specialist workers — Curator, Janitor, Distiller, and Surveyor — each with strictly enforced scope permissions. They interact with your vault only through a controlled CLI gate; they never touch the filesystem directly.
...
You don't have to hand everything to Alfred manually. Streams are data pipelines that capture events from your email, calendar, payment providers, and ambient devices — delivering them to Alfred's Inbox automatically. The Curator processes each event just as if you'd shared the content yourself.

<Card title="Data Layer" icon="wave-pulse" href="/architecture/data">
  Stream types, the StreamEvent envelope, managing streams, and session logs.
</Card>
...
The vault is an Obsidian-compatible collection of Markdown files. Alfred manages 22 record types across four layers: standing entities, activity records, learning types, and intuition types. Records connect through wikilinks, forming a growing knowledge graph. Over time, Alfred develops **intuition** — learned patterns that let it handle routine decisions on your behalf.
...
Temporal is the workflow engine. It orchestrates the specialists, the intuition processes, and (coming soon) Tasks, Chores, and Rules — one-off actions, recurring jobs, and if/then automation that turn vault knowledge into proactive service.

<Card title="Kinetic Layer" icon="gears" href="/architecture/kinetic">
  Temporal engine, Tasks, Chores, Rules, and the twelve background processes.
</Card>
...
Every way you interact with Alfred — the dashboard, OpenClaw channels (WhatsApp, iMessage, Telegram, Discord), voice calls via Alfred Talk, continuous listening via Ambient Mode, and the REST API. Every interface feeds into the same pipeline.

<Card title="Interface Layer" icon="display" href="/architecture/interface">
  Dashboard, channels, Talk Mode, Ambient Mode, device pairing, and the API.
</Card>
